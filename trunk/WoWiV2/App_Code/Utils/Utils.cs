...
public class Utils
{
  ...
  /// <summary>
  /// PR Project DropDownList專用，Value取的欄位是：Project_No
  /// </summary>
  ...
  /// <summary>
  /// 成本分析Project DropDownList專用，Value取的欄位是：Project_Id
  /// </summary>
  /// <param name="sender"></param>
  /// <param name="e"></param>
  public static void CostAnalysisProjectDDL_Load(object sender, EventArgs e)
  { 
    (sender as DropDownList).DataSource =
      from c in wowidb.Projects
      orderby c.Project_No descending
      select new
      {
        Project_No = c.Project_No +
          " - [" + ((from qq in wowidb.Quotation_Version from aa in wowidb.clientapplicants where qq.Quotation_No == c.Quotation_No & qq.Applicant_Id == aa.id select aa.companyname).FirstOrDefault()) +
          "] - [" + ((from qq in wowidb.Quotation_Version where qq.Quotation_No == c.Quotation_No select qq.Model_No).FirstOrDefault()) + "]",
