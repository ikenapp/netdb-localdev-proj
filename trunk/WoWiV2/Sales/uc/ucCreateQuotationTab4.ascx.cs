...
public partial class Sales_uc_ucCreateQuotationTab4 : System.Web.UI.UserControl
{
    ...
    private void LoadData(int quotation_id)
    {
        ...
        txtTargetTotalPrice.Text =  (quo.TargetTotalPrice == null)?"0": ((Decimal)quo.TargetTotalPrice).ToString("F2");
        txtDiscount.Text = (quo.Discount==null)?"0":((Decimal)quo.Discount).ToString("F2");
        txtFinalTotalPrice.Text = (quo.FinalTotalPrice==null)?"0":((Decimal)quo.FinalTotalPrice).ToString("F2");
        txtTotal_Disc_Amt.Text = (quo.Total_disc_amt==null)?"0":((Decimal)quo.Total_disc_amt).ToString("F2");
